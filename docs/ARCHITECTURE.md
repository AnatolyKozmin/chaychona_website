...
| `Course`, `CourseBlock`, `CourseSubBlock`, `CourseBlockSlide`, `CourseBlockProgress` | `models/course.py` | учебные курсы (стандарты), привязка к ресторану/должности, прогресс прохождения по блокам. У блока есть `kind`: `text` — текст+картинка+подблоки, `deck` — колода слайдов презентации в `CourseBlockSlide` (картинки, нарезанные из PDF при заливке) |
...
| `MenuBranch`, `MenuCategory`, `MenuDish`, `MenuDishMediaJob`, `MenuImportSession`, `MenuImportRow` | `models/menu.py` | меню ресторана: категории, блюда (фото/аудио/видео/ингредиенты/цена) — раздел «Вкусная тетрадь»; `MenuDishMediaJob` — очередь фоновой генерации медиа в три вида (`image` — картинка ингредиентов, `audio` — озвучка, `video` — склейка mp4), таблица исторически называется `menu_dish_video_jobs`, алиас `MenuDishVideoJob` оставлен для старого кода; `MenuImportSession` + `MenuImportRow` — залив меню файлом и построчный отчёт по нему |
...
- **tests.py** — CRUD тестов, прохождение (`/my`, `/{id}/take`, `/{id}/submit`), история попыток (`/my-attempts`), аналитика (`/analytics`), выгрузка результатов в Excel (`GET /analytics/export`, фильтры ресторан/должность/тест/период по Москве; сборка книги — `app/services/analytics_export.py`, листы: попытки, сотрудники, рестораны, тесты, сложные вопросы, не проходили), импорт из Excel/Word (`/import-xlsx`, `/import-docx`, `/parse-docx`, `/import-apply`, `/import-template`)
...
- **menu.py** — публичная лента меню (`/feed`, `/categories`), admin CRUD по branches/categories/dishes, загрузка медиа; залив всего меню одним файлом (`POST /admin/import`, multipart: `.xlsx` или `.zip`; `dry_run=true` — только план) с отчётом и прогрессом (`GET /admin/import`, `GET /admin/import/{id}`, `POST /admin/import/{id}/retry`); пачечная загрузка одного блюда с постановкой видео в очередь (`POST /admin/dishes/import-job`, multipart); массовый/одиночный backfill видео для уже существующих блюд (`POST /admin/dishes/generate-videos`, `POST /admin/dishes/{id}/generate-video`); статус очереди (`GET /admin/dishes/import-jobs`)
...
- `MAGNIFIC_API_KEY`, `MAGNIFIC_API_BASE`, `MAGNIFIC_IMAGE_MODEL` — генерация картинки ингредиентов при заливе меню
- `ELEVENLABS_API_KEY`, `ELEVENLABS_API_BASE`, `ELEVENLABS_VOICE_ID`, `ELEVENLABS_MODEL` — озвучка
- `GENERATION_TIMEOUT_SECONDS` — потолок ожидания внешнего провайдера на одно задание
...
- **Меню целиком, через сайт**: суперадмин заливает файл в разделе «Вкусная тетрадь» → «Загрузка меню файлом». Принимается `.docx` («тетрадь» шефа: текст и фотографии блюд внутри документа, разбор — `app/services/menu_docx_import.py`), `.xlsx` (только текст) или `.zip`, внутри которого тот же реестр вместе с папками медиа. Формат реестра — [docs/menu_registry_format.md](menu_registry_format.md). Разбор — `app/services/menu_import.py`, запись в базу — `app/services/menu_import_runner.py`, endpoint — `POST /menu/admin/import`. Ресторан-получатель обязателен: либо выбрать существующий, либо ввести название нового (создастся). Текст и файлы из архива пишутся синхронно, генерация недостающего медиа уезжает в очередь.
...
- **Презентации в стандарты**: админ выбирает у блока вид «Презентация (PDF)» и заливает файл в разделе «Стандарты» → «Управление стандартами». Сервер (`app/services/presentation.py`, endpoint `POST /courses/admin/presentation`) режет PDF на картинки-слайды и отдаёт их пути; к стандарту они привязываются при сохранении формы. Принимается только PDF — `.pptx` потребовал бы LibreOffice в образе и молча портил бы вёрстку на нестандартных шрифтах; из PowerPoint надо сохранить в PDF.
...
- Нет Alembic — миграции схемы это код в `main.py::_run_startup`, выполняется на каждом старте backend. Новые таблицы (`menu_dish_video_jobs`, `menu_import_sessions`, `menu_import_rows`) создаются штатным `Base.metadata.create_all`, а колонки, добавленные к уже существующим таблицам (`kind`, `prompt`, `session_id` у `menu_dish_video_jobs`, `kind` у `course_blocks`), — idempotent `ALTER` там же.
- Презентация в стандарте — это картинки, а не PDF: сотрудник листает нарезанные слайды тем же свайпом, что и обычные блоки, тап открывает слайд на весь экран (иначе мелкий текст с телефона не прочитать). Отдавать сам PDF нельзя — мобильный браузер либо тянет тяжёлый рендерер, либо передаёт файл во внешнюю читалку вместе с сотрудником, и прогресс теряется. Рендер — `pypdfium2` + `Pillow` (самодостаточные колёса, apt-пакетов в образ не надо), ограничения — `MAX_SLIDES`, `TARGET_WIDTH_PX` в `app/services/presentation.py` и `MAX_PRESENTATION_SIZE_BYTES` в `api/v1/courses.py`. Блок-колоду нельзя отметить изученным, пока сотрудник не долистал до последнего слайда.
- Числа в `presentation.py` подобраны по замерам, а не на глаз: `TARGET_WIDTH_PX = 2000` — минимум, при котором сноска 9-12 пт со слайда 13.3" читается с телефона на увеличении 260% (на 1600 мылит); `SIZE_SOFT_LIMIT_BYTES` + `JPEG_QUALITY_HEAVY` дожимают только слайды-фотографии, которые иначе уезжают на мобильную сеть под мегабайт, — текстовых страниц это не касается. Тесты на оба факта — `backend/tests/test_course_presentation.py`.
- Просмотрщик колоды в `StandardsStudyView.vue` — карусель из трёх ячеек (предыдущая/текущая/следующая), абсолютно спозиционированных и сдвинутых на ±100%: соседи уже загружены, поэтому свайп не моргает, а сотня слайдов в DOM не висит. На ширине до 768px `.deck-stage` выходит из отступов `.card` и `.tinder-card` (`calc(100% + 76px)`), иначе слайду доставалось бы 72% ширины экрана. Полноэкранный режим — это режим чтения: из него листают слайды, уровень увеличения при этом сохраняется.
- Сохранение курса пересоздаёт блоки целиком (`_replace_blocks`), поэтому форма админки обязана возвращать на сервер всё, что у блока было: и `image_path`, и `slides`. Что не отправлено обратно — из стандарта пропадёт вместе со старыми id.
- Генерация медиа блюд — фоновый воркер `app/services/video_worker.py` (демон-поток, стартует в `lifespan`): забирает `pending`-задания из `menu_dish_video_jobs` через `FOR UPDATE SKIP LOCKED` и выполняет их по `kind` — `image` рисует картинку ингредиентов через Magnific, `audio` озвучивает текст через ElevenLabs (`app/services/generation.py`), `video` склеивает mp4 через `ffmpeg` (`app/services/video.py`). Видео зависит от двух предыдущих стадий, поэтому создаётся в статусе `blocked` и переводится в `pending` тем же воркером, когда у блюда появляются оба файла; если смежные стадии отработали, а файлов нет, задание закрывается ошибкой и не висит в очереди вечно. Требует `ffmpeg` в образе (стоит в `backend/Dockerfile`). Настройки — `FFMPEG_BINARY`, `VIDEO_WORKER_ENABLED`, `VIDEO_WORKER_POLL_SECONDS`, `VIDEO_WORKER_TIMEOUT_SECONDS`. Для уже загруженных блюд видео ставится кнопкой «Сгенерировать видео для всех» в админке «Вкусной тетради» (`TastyNotebookView.vue` → `POST /admin/dishes/generate-videos`).
...
- Прод-секреты (`JWT_SECRET_KEY`, `BOOTSTRAP_SUPERADMIN_PASSWORD`, при желании `BOOTSTRAP_SUPERADMIN_EMAIL` и ключи генерации) берутся из `.env` в корне репозитория на сервере — compose читает его сам, файл в `.gitignore`. Для первых двух дефолта нет намеренно: без них `docker compose up` падает, а не стартует с известным всем значением из публичного репозитория. Смена `JWT_SECRET_KEY` разлогинивает всех. Смена `BOOTSTRAP_SUPERADMIN_EMAIL` заведёт нового суперадмина, а старый останется со своим последним паролем.
