...
- **Меню целиком, через сайт**: суперадмин заливает файл в разделе «Вкусная тетрадь» → «Загрузка меню файлом». Принимается `.xlsx` (только текст) или `.zip`, внутри которого тот же реестр вместе с папками медиа. Формат реестра — [docs/menu_registry_format.md](menu_registry_format.md). Разбор — `app/services/menu_import.py`, запись в базу — `app/services/menu_import_runner.py`, endpoint — `POST /menu/admin/import`. Ресторан-получатель обязателен: либо выбрать существующий, либо ввести название нового (создастся). Текст и файлы из архива пишутся синхронно, генерация недостающего медиа уезжает в очередь.
