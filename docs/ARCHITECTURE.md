...
- Нет Alembic — миграции схемы это код в `main.py::_run_startup`, выполняется на каждом старте backend. Новые таблицы (`menu_dish_video_jobs`, `menu_import_sessions`, `menu_import_rows`) создаются штатным `Base.metadata.create_all`, а колонки, добавленные к уже существующим таблицам (`kind`, `prompt`, `session_id` у `menu_dish_video_jobs`), — idempotent `ALTER` там же.
